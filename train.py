...
def validate(network, eval_loader, weight, *criterion):
    ...
    with torch.no_grad():
        ...
        for j, batch_eval in enumerate(eval_loader.get_dataloader()):
            ...
            stage1_loss = criterion[0](outputs[0], batch_eval)
            stage2_loss = criterion[1](outputs[1], batch_eval)
...
if __name__ == '__main__':
    ...
    stage1_criterion = wavemse_nextFrameNoise_loss()
    stage2_criterion = pcm_loss(config['WIN_LEN'], config['WIN_OFFSET'])
    weight = [1.]
    ...
    for epoch in range(start_epoch, config['MAX_EPOCH']):
        ...
        for i, batch_info in enumerate(tr_batch_dataloader.get_dataloader()):
            ...
            optimizer.zero_grad()
            outputs = network(features)
            stage1_loss = stage1_criterion(outputs[0], batch_info)  # 0.4809
            stage2_loss = stage2_criterion(outputs[1], batch_info)  # 2.3346
            loss = stage1_loss + stage2_loss
            loss.backward()
            optimizer.step()
            ...
            if config['USE_CV'] and (i + 1) % config['EVAL_STEP'] == 0:
                ...
                avg_eval_loss = validate(network, cv_batch_dataloader, weight, stage1_criterion, stage2_criterion)
